...
public:
    ...
    runtime(sim_duration), report_output(report)
  {
    ...
    if (nurses == 1) {
      DoctorQueue *nurse1 = new DoctorQueue(random, reception, min_treatment_time, max_treatment_time, report_output);
      queues.push_back(nurse1);
    }
    if (nurses == 2) {
      DoctorQueue *nurse1 = new DoctorQueue(random, reception, min_treatment_time, max_treatment_time, report_output);
      DoctorQueue *nurse2 = new DoctorQueue(random, reception, min_treatment_time, max_treatment_time, report_output);
      queues.push_back(nurse1);
